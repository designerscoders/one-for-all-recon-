...
cat /tmp/curl.out | grep $TARGET | grep TD | sed -e 's/<//g' | sed -e 's/>//g' | sed -e 's/TD//g' | sed -e 's/\///g' | sed -e 's/ //g' | sed -n '1!p' | sort -u > $TARGET.txt
cat $TARGET.txt
echo -e "$OKRED [+] Domains saved to: $TARGET.txt"
