...
worlist="/root/tools/SecLists/Discovery/DNS/bitquark-subdomains-top100000.txt"
...
domain_enum(){
	...
	do
        ...
        cd /root/basic-recon/$domain/Recon/vulnerabilities && touch dalfox_xss.txt && cd  /root/basic-recon/

        cd /root/basic-recon/$domain/Recon/sources && touch bucket.txt && cd  /root/basic-recon/
		...
		./crt.sh $domain && mv $domain.txt $domain/sources/crt.txt
...
http_prob(){
	...
	do
		...
		cat $domain/domains.txt | httprobe | tee $domain/Recon/url.txt
...
DNSMAP_start(){
   ...
   dnsmap $domain -r $domain/sources/dnsmap.txt

   sed -i '/^$/d' $domain/sources/dnsmap.txt
...
DNSEnum(){
   ...
   dnsenum --noreverse -o $domain/sources/DNSEnum-All.txt $domain --private
...
scanner(){
	...
	do
		...
		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/cves  -c 332 -o  $domain/Recon/nuclei/cves.txt

		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/default-logins/ -c 332 -o  $domain/Recon/nuclei/default-logins.txt

		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/dns/ -c 332 -o  $domain/Recon/nuclei/dns.txt
		
	    cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/cnvd -c 332 -o  $domain/Recon/nuclei/cnvd.txt 

	    cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/file -c 332 -o  $domain/Recon/nuclei/file.txt               

	    cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/iot -c 332 -o  $domain/Recon/nuclei/iot.txt               

	    cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/miscellaneous -c 332 -o  $domain/Recon/nuclei/miscellaneous.txt               

	    cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/misconfiguration  -c 332 -o  $domain/Recon/nuclei/misconfiguration.txt               
	    
		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/exposed-panels/ -c 332 -o  $domain/Recon/nuclei/exposed-panels.txt

		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/exposures/ -c 332 -o  $domain/Recon/nuclei/exposures.txt
 
		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/network/ -c 332 -o  $domain/Recon/nuclei/network.txt

		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/takeovers/ -c 332 -o  $domain/Recon/nuclei/takeovers.txt

		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/technologies/ -c 332 -o  $domain/Recon/nuclei/technologies.txt

		cat $domain/Recon/url.txt | nuclei -t /root/tools/nuclei-templates/vulnerabilities/ -c 332 -o  $domain/Recon/nuclei/vulnerabilities.txt

		find ~/basic-recon/$domain/Recon/nuclei -empty -type f -delete
...
gf_patterns(){
	...
	do
		...
		find ~/basic-recon/$domain/Recon/gf -empty -type f -delete
...
hidden(){
	...
	do
		...
		dirsearch -u $domain -t 100 -o /root/basic-recon/$domain/Recon/hidden_dir/$domain.txt
...
API_secret(){
	...
	do
	  ...
	  printf "\n\n---------------------------------------------\n"
        ...
        find ~/basic-recon/$domain/Recon/API_secret/ -empty -type f -delete
...
api_domains(){
	...
	do
		printf "\n\n---------------------------------------------\n"
		printf "          \e[33mapi_domain finding ...\e[0m                  "
		printf "\n---------------------------------------------\n\n"

        cat $domain/domains.txt | grep api | tee /root/basic-recon/$domain/Recon/API_secret/api_domain.txt
...
find_xss(){
	...
	do
        ...
        cat $domain/Recon/gf/xss.txt | grep "source=" |  qsreplace '"><script>confirm(1)</script>' | while read host do ; do curl --silent --path-as-is --insecure "$host" | grep -qs "<script>confirm(1)" && echo "$host \033[0;31mVulnerable\n" || echo "$host \033[0;32mNot Vulnerable\n" | >>  /root/basic-recon/$domain/Recon/vulnerabilities/sourse_xss.txt ; done               
...
Heardbleeds_vulnerability(){
	...
	do
        ...
        cat $domain/Recon/url.txt | while read line ; do echo "QUIT" | openssl s_client -connect $line:443 2>&1 | grep 'server extension "heartbeat" (id=15)' || echo $line: safe  >> /root/basic-recon/$domain/Recon/vulnerabilities/heardbled.txt | grep safe; done
...
github_secrets(){
	...
	do
		...
		./git-hound --subdomain-file /root/basic-recon/$domain/domains.txt  --pages 10  >>  /root/basic-recon/$domain/Recon/git-dorking/dorking.txt                  
...
bucket_finder(){
	...
	do
		printf "\n\n---------------------------------------------\n"
		printf "          \e[33mbucketfinding  Is Starting ...\e[0m                  "
		printf "\n---------------------------------------------\n\n"

		cd /root/tools/lazys3

		ruby lazys3.rb $domain >> /root/basic-recon/$domain/sources/bucket.txt
...
clickjacking(){
	...
	do
		printf "\n\n---------------------------------------------\n"
		printf "          \e[33 clickjacking Is Starting ...\e[0m                  "
		printf "\n---------------------------------------------\n\n"

		cd /root/tools/Clickjacking-Tester

		python3 Clickjacking_Tester.py /root/basic-recon/$domain/Recon/url.txt | tee /root/basic-recon/$domain/Recon/vulnerabilities/clickjacking.txt

		cd /root/basic-recon/$domain/Recon/vulnerabilities/

        cat clickjacking.txt | cut -c 5- > clickjacking.txt
...
open_redirect(){
	...
	do
		printf "\n\n---------------------------------------------\n"
		printf "          \e[33mopen redirect finding ...\e[0m                  "
		printf "\n---------------------------------------------\n\n"

       cat /root/basic-recon/$domain/Recon/wayback/wayback.txt | grep -a -i \=http | qsreplace 'http://evil.com' | while read host do;do curl -s -L $host -I | grep "evil.com" && echo "$host\033[0;31mvulnerable\n" | tee /root/basic-recon/$domain/Recon/vulnerabilities/open-redirect.txt ; done 
...
eyewitness_api(){
	...
	do
		...
		printf "\n\n-------------------------------------------------------------\n"
		printf "          \e[33mapi domain scrren short..\e[0m                  "
		printf "\n---------------------------------------------------------------\n\n"



		eyewitness -f $domain/domains.txt/api.txt --web
...
eyewitness(){
	...
	do
		...
		eyewitness -f /root/basic-recon/$domain/domains.txt --web
